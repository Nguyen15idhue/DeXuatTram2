...
const SYSTEM_PROMPT = [
  ...
  '4. CHỈ dùng thông tin trong tài liệu được cung cấp. Tuyệt đối không bịa. Nếu thiếu thông tin, nói rõ là không tìm thấy.',
...
async function articlesVersion() {
  ...
  return `${rows[0].n}:${new Date(rows[0].v).getTime()}`;
}
...
function buildUserPrompt({ question, articles, knowledge, history, tokens, dataContext }) {
  const blocks = [];
  ...
  blocks.push(...articles.map((a, i) => {
    const body = selectSections(htmlToStructuredText(a.content_html), tokens, SNIPPET_CHARS);
  ...
  if (knowledge && knowledge.length > 0) {
    ...
    blocks.push(...knowledge.map((k, i) => `[K${i + 1}] ${k.heading}\n${k.content.slice(0, SNIPPET_CHARS)}`));
  }
...
async function log({ question, answer, sources, provider, latencyMs, fallbackReason, userId }) {
  try {
    await pool.query(
      'INSERT INTO assistant_logs (question, answer, sources, provider, latency_ms, fallback_reason, user_id) VALUES (?, ?, CAST(? AS JSON), ?, ?, ?, ?)',
      [truncate(question, 500), answer || null, JSON.stringify(sources || []), truncate(provider, 20), latencyMs || null, truncate(fallbackReason, 255), userId || null]
    );
  } catch (err) {
...
async function ask(question, user, rawHistory) {
  ...
  if (articles.length === 0 && knowledge.length === 0 && !dataContext) {
    const result = {
      answer: 'Mình không tìm thấy nội dung này trong tài liệu hướng dẫn. Bạn thử diễn đạt khác, hoặc xem mục lục trang Hướng dẫn để tìm bài gần nhất nhé.',
      sources: [],
      knowledge: [],
      provider: null,
      latencyMs: 0,
    ...
    ttlCache.set(cacheKey, result, CACHE_TTL_MS);
    await log({ question: q, answer: result.answer, sources: [], provider: null, latencyMs: 0, fallbackReason: 'no_sources', userId: user?.id });
    return result;
  }
