...
const KET_QUA_FILE = 'test-deploy-step3-results.json';
...
async function main() {
  ...
  try {
    ...
    await page.waitForTimeout(2500);
    const coBang = await page.locator('table').count();
    ghi('Trang quan ly users hien thi bang', coBang > 0, `table = ${coBang}`);
    const coAdmin = await page.getByText('admin@station.com', { exact: false }).count();
    ghi('Danh sach co tai khoan admin', coAdmin > 0, `"admin@station.com" = ${coAdmin}`);
