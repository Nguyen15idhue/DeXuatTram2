...
const FileViewer = ({ file, onClose }) => {
  ...
  const isEmbeddable = isPdf || mime.includes('word') || mime.includes('excel') || mime.includes('sheet');
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 800, maxHeight: '90vh' }}>
        ...
        <div style={{ overflow: 'auto', maxHeight: 'calc(90vh - 80px)', background: '#000', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 200 }}>
          {isImage && (
            <img
              src={url}
          ...
          {!isImage && !isVideo && !isAudio && !isEmbeddable && (
            <div style={{ padding: 40, textAlign: 'center', color: '#999' }}>
              <div style={{ fontSize: 48, marginBottom: 16 }}>📁</div>
              <p>Không thể xem trước định dạng này</p>
              <a href={url} target="_blank" rel="noopener noreferrer" className="btn btn-primary" style={{ marginTop: 8 }}>
                Tải về để xem
              </a>
            </div>
          )}
